"""Evaluate both checkpoints on ENVSDD_test (full test set including TUTASC2019)."""
...
def eval_and_print(name, score, binary):
    ...
    print(f"    Real%={real_acc*100:.2f}%  Fake%={fake_acc*100:.2f}%")
...
checkpoints = {
    ...
    "retrain_run1_ep06": {
        "ckpt": "checkpoint/Beats_journal/retrain_cclW8_ceW3_ceRW8_LR5e-08_8ep/epoch-06_bal0.9653.ckpt",
        "ref_fake": "data/label/beats/Event_train_stage1_fakeonly.json",
    },
    "Combined (Event+TUTASC19)": {
        "ckpt": "checkpoint/Beats_journal/combined_Event_TUTASC19_scratch/stage2/S2r_ep02.ckpt",
        "ref_fake": "data/label/beats/combined_Event_TUTASC19_train_fakeonly.json",
    },
}

test_json = "data/label/beats/ENVSDD_test.json"
...
for ckpt_name, cfg in checkpoints.items():
    ...
    pipeline = load_model(cfg["ckpt"])
    ref_fake_bn, _, _ = collect(pipeline, cfg["ref_fake"])
    ...
    score = -loglik(test_bn, gF)
    eval_and_print("baseline (-LL_fake)", score, binary)
