...
from src.ENVIT.ENVIT_dataset import ENVITDataset
...
import timm
# from src.utils.training_utils import apply_reshape, feature_extract


def load_pipeline(checkpoint_path=None, device='cuda', num_classes=2, args=None):
    ...
    try:
        ...
        pipeline = EfficientViT(channels=1280, selected_efficient_net = 0, args=args).to(device)
...
def predict_single(pipeline, input_tensors, device):
    with torch.no_grad():
        ...
        logits = pipeline.forward(input_tensors)
...
def predicted_batch(pipeline, dataloader, device='cuda', feature_extractor=None, new_H=2048):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            logits = pipeline.forward(inputs)
            probability = F.softmax(input=logits, dim=1)
            predicted_class = torch.argmax(input=probability, dim=1)
            confidence = torch.max(input=probability, dim=1)[0]
...
if __name__ == '__main__':
    ...
    ckpt_path = f'checkpoint/ENVIT_stage2/sample-epoch=01-valid/loss=3.17.ckpt'
    ...
    baseline_dataset = ENVITDataset(json_file=json_file, transformation=None, args=args)
    
    pipeline = load_pipeline(checkpoint_path=ckpt_path,args=args)
    ...
    if 'labels' in results:
        ...
        # ====================================================
        # Create per-audio rows (7800 rows)
        # ====================================================
