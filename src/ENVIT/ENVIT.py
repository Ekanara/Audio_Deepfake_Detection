...
class EfficientViT(nn.Module): 
    def __init__(self, channels=1280, selected_efficient_net = 0, args = None):
        super().__init__() 
        self.args = args
        image_size =self.args.resolution
        patch_size = self.args.patch_size
        num_label = self.args.num_label
        dim = self.args.dim
        depth = self.args.depth
        heads = self.args.heads
        mlp_dim = self.args.mlp_dim
        emb_dim = self.args.emb_dim
        dim_head = self.args.dim_head
        ...
        self.mlp_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(mlp_dim, num_label)
        )

    def reset_classifier(self, num_label=2):
        self.mlp_head = nn.Sequential(
            nn.Linear(self.args.dim, self.args.mlp_dim),
            nn.ReLU(),
            nn.Linear(self.args.mlp_dim, num_label)
        )
