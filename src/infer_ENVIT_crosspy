...
import timm
from other_repo.ENVIT.efficient_vit.efficient_vit import EfficientViT
from utils.training_utils import apply_reshape, feature_extract
import yaml
#from other_repo.ENVIT.efficient_vit.transforms.albu import IsotropicResize
#from other_repo.ENVIT.efficient_vit.test import create_base_transform
import cv2
import torchvision
...
def load_pipeline(checkpoint_path=None, config=None, efficient_net=0):
    ...
    if efficient_net == 0:
        ...
        channels = 2560
    try:
        pipeline = EfficientViT(config=config, channels=channels, selected_efficient_net = efficient_net)
        pipeline.load_state_dict(torch.load(checkpoint_path))
        pipeline.eval()
...
def predicted_batch(pipeline, dataloader, device='cuda', feature_extractor=None, new_H=2048):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            # # Move to CPU and make a grid of images
            # grid = torchvision.utils.make_grid(inputs.cpu().float(), nrow=8, normalize=True, scale_each=True)
            ...
            probability = F.softmax(input=logits, dim=1)
            #probability = F.sigmoid(input=logits)
            predicted_class = torch.argmax(input=probability, dim=1)
            confidence = torch.max(input=probability, dim=1)[0]
...
def evaluate_model(results, class_names=None):
    ...
    if num_classes == 2:
        ...
        if probabilities.ndim == 1 or probabilities.shape[1] == 1:
            # Binary with only positive-class probability
            auc_score = roc_auc_score(labels, probabilities.ravel())
        else:
    ...
    if num_classes == 2:
        # Binary ROC curve
        if probabilities.ndim == 1 or probabilities.shape[1] == 1:
            fpr, tpr, _ = roc_curve(labels, probabilities.ravel())
        else: 
...
if __name__ == '__main__':
    ...
    ckpt_path = f'other_repo/ENVIT/efficient_vit/efficient_vit.pth'
    config = f'other_repo/ENVIT/efficient_vit/configs/architecture.yaml'
    efficcient_net = 2
    ...
    transformation = A.Compose([
        A.LongestMaxSize(max_size=224),       # resize longest side to 224
    ...
    pipeline = load_pipeline(checkpoint_path=ckpt_path, config=config, efficient_net=0)
