...
class BaselineTrainer():
    ...
    def create_trainer(self, log_model = False):
        os.system(f"wandb login --relogin {self.args.wandb_api}")
        wandb_logger = WandbLogger(
            project=self.args.wandb_run_name,
            log_model=log_model,
        )
        ...
        model_checkpoint = ModelCheckpoint(
            ...
            save_weights_only=True if self.args.save_weights_only else False,
        )

        dist = True if len(self.args.devices) > 1 else False
    ...
    def prepare_dataloader(self, json_file = None ,sampler = None, device = 'cuda', streaming = True):
        if json_file == None:
        ...
        dataloader = torch.utils.data.DataLoader(
            train_dataset,
            batch_size = batch_size if sampler == None, else: None
            batch_sampler = sampler,
    ...
    def train_stage1(self, trainer, device, dist, model, feature_extractor, pipeline):
        samples_per_label = {'efs': 32, 'fr': 32, 'fs': 32, 'real': 32}  
        sampler = BalancedGeneratorSampler(training_dataset, samples_per_label)

        train_dataloader = self.prepare_dataloader(sampler = sampler, device = device)

        if self.args.do_eval:
            val_dataloader = self.prepare_dataloader(streaming = False)
        else: val_dataloader = None

        featutre_extractor = feature_extractor
    ...
    def train_stage2(self, checkpoint = None):
        if checkpoint == None:
