...
from src.ENVIT.ENVIT_dataset import BaselineDataset, BalancedGeneratorSampler, BalancedGeneratorSampler_Stage2, BalancedBatchSampler
...
def main(args):
    ...
    if args.do_train:
        ...
        training_dataset = BaselineDataset(json_file=json_file, transformation=None)

        samples_per_label = {'fake_ata_01': 32, 'fake_tta_01': 32, 'fake_tta_02': 32, 'fake_tta_03': 32, 'real': 32}  
    ...
    if args.do_train_stage2:
        ...
        training_dataset = BaselineDataset(json_file=json_file, transformation=None)
        samples_per_label = {'fake':64 , 'real': 64}  
        sampler = BalancedGeneratorSampler(training_dataset, samples_per_label)
    ...
    if args.do_train:
        ...
        pipeline = EfficientVit(args=args)
        pipeline.train()
    ...
    if args.do_train_stage2:
        ckpt_path = f'checkpoint/Audio_Deepfake_Detection_stage1/checkpoint/best.pt'
        checkpoint = torch.load(ckpt_path, map_location=device)
        
        # Recreate the model architecture
        model = efficientnet_b1(pretrained=False)  # Don't load audioNet weights
        model.train()
        pipeline = BasePipeline( 
            model=model, 
            device=device, 
            in_features=3, 
            num_label=5  # Original number of classes from training
        )
