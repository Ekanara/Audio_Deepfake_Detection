...
class BasePTLN(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # if audio.shape[1] != 3 and audio.shape[-1] == 3:  # if 
        #     audio = audio.permute(0, 3, 2, 1)

        # print(f"Model device: {next(self.parameters()).device}")
        # print(f"Input device: {batch['audio'].device}")
        # print(f"Label device: {batch['label'].device}")
        # print(f"Current device: {self.device}")
        # model_pred, target, timesteps = self.forward(model_input, audio, mask, original_size, crop_top_lefts, noise)
        #print(audio.shape)
