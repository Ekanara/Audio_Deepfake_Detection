...
class EfficientNetPTLN(LightningModule):
    def __init__(self, args, sync_dist=None, pipeline=None, feature_extractor=None):
        super().__init__()
        self.args = args
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.feature_extractor:
            image = feature_extract(input=image, feature_extractor=self.feature_extractor)
            image = apply_reshape(input=image)

            B, C, H, W = image.shape
            new_H = 512
            new_W = (H*W)//new_H
            image = image.reshape(B,C,new_H,new_W)
