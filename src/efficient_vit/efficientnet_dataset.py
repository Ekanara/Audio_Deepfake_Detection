...
class EfficientNetDataset(BaselineDataset):
    def __init__(self, json_file, transformation=None, device='cuda'):
        super().__init__(json_file=json_file, transformation=transformation, device=device)

    def __getitem__(self,idx):
        dataset = super().__getitem__(idx)
        return dataset
...
class BalancedGeneratorSampler(Sampler):
    def __init__(self, dataset, samples_per_label):
        ...
        for idx, item in enumerate(dataset.data):
            ...
            gen_name = os.path.basename(os.path.dirname(item['image']))  # subfolder name
            ...
            for label in self.labels:
                ...
                while picked < self.samples_per_label[label]:
                    for gen in gens:
                        ...
                        imgs = self.label_to_gen_indices[label][gen]
                        if ptr >= len(imgs):
                            continue  # skip if generator is empty
                        batch.append(imgs[ptr])
    ...
    def __len__(self):
        return min(
            sum(len(imgs) for imgs in self.label_to_gen_indices[label].values())
        ...
        while True:
            ...
            for gen in self.generators:
                for _ in range(self.samples_per_generator):
                    ...
                    imgs = self.fake_gen_to_indices[gen]
                    if ptr >= len(imgs):
                        return
                    batch.append(imgs[ptr])
...
if __name__ == '__main__':
    ...
    dataset = EfficientNetDataset(json_file=json_file, model = model, transformation=transformation)
